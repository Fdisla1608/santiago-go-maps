...
const Cards = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `http://swift-agro.ddns.net:3001/api/cards/transaction?fkTarjeta=${CardData.idCard}`
      );
      if (!response.ok) {
  ...
  const ScanCard = async (params) => {
    try {
      setBalance(0);
      const response = await fetch(
        `http://swift-agro.ddns.net:3001/api/cards/card?cod=${params}`
      );
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
  ...
  const InsertTransaction = async () => {
    try {
      const response = await fetch(
        `http://swift-agro.ddns.net:3001/api/cards/transaction`,
        {
          method: "POST",
          headers: {
